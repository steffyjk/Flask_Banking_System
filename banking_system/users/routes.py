...
from banking_system.models import Account, Branch, Card, FixedDeposit, Insurance, InsuranceType, Loan, LoanType, Transaction, TransactionType, User, UserType
from banking_system.users.forms import AddMoney, RegistrationForm, LoginForm, UpdateAccountForm,RequestResetForm,ResetPasswordForm
    ...
    if current_user.is_authenticated:
        ...
        if account:
            ...
            if card:
                ...
                card = Card(card_number=card_number,cvv_number=cvv_number,card_pin=card_pin,expiry_date=expiry_date,account_number=account_number)
                db.session.add(card)
                db.session.commit()
